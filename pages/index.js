...
export default function Анкета() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Подготовка данных: преобразуем числа и очищаем пустые строки при необходимости
    const payload = { ...formData };

    // Преобразуем "Койко-дни стационар" в число, если указано
    if (payload['Койко-дни стационар'] !== '') {
      const num = Number(payload['Койко-дни стационар']);
      if (!isNaN(num)) {
        payload['Койко-дни стационар'] = num;
      }
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {status === 'error' && (
            <p className="text-red-600 text-center mt-4">
              ❌ Ошибка при отправке. Проверьте консоль браузера или свяжитесь с администратором.
            </p>
          )}
        </form>
      </div>
